...
OBJLoader::OBJLoader (const char* file) : mFileName (file), mVertices(NULL), mTextureCoordinates(NULL), mNormals(NULL), mIndices(NULL), nIndices(0) {}

OBJLoader::~OBJLoader () {
	if (NULL != mVertices) delete [] mVertices;
	if (NULL != mTextureCoordinates) delete [] mTextureCoordinates;
	if (NULL != mNormals) delete [] mNormals;
	if (NULL != mIndices) delete [] mIndices; }
